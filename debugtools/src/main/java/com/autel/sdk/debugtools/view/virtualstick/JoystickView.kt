...
class JoystickView(context: Context?, attrs: AttributeSet) : SurfaceView(context, attrs),
    SurfaceHolder.Callback, OnTouchListener {
    private var tag = "OnScreenJoystick"
    ...
    private var mJoystickListener: OnJoystickListener? = null
    ...
    fun setJoystickListener(pJoystickListener: OnJoystickListener?) {
        ...
        when (pEvent.action) {
            MotionEvent.ACTION_UP -> if (isAutoCentering) {
                ...
                if (checkBounds(x, y)) {
                    ...
                    val angle = atan2((y - mRadius).toDouble(), (x - mRadius).toDouble())
                    mKnobX =
