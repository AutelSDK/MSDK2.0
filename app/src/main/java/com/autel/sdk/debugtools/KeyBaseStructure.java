...
//     */
    ...
    protected void get(AutelKeyInfo<R> keyInfo, CommonCallbacks.CompletionCallbackWithParam<R> getCallback) {
        ...
        if (getKeyManager(key) == null) {
            ToastUtils.INSTANCE.showToast(SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_keymanager_null));
        } else {
    ...
    public void cancelAllListen() {
        ...
        if (device == null || DeviceManager.Companion.getDeviceManager().getFirstRemoteDevice() == null) {
            ToastUtils.INSTANCE.showToast(SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_device_null));
        } else {
