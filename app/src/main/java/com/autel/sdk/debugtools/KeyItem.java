...
public class KeyItem<P, R> extends KeyBaseStructure implements Comparable<KeyItem<?, ?>> {
    ...
    private void start(int type, String param) {
        if (keyOperateCallBack != null) {
            ...
            switch (type) {
                case GET:
                    keyOperateCallBack.actionChange(SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_send)
                            + "：【GET】 【" + keyInfo.keyName + "】 "
                            + SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_parameter)
                            + param);
                    break;
                case SET:
                    keyOperateCallBack.actionChange(SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_send)
    ...
    private void endSuccess(int type, String result) {
        if (keyOperateCallBack != null) {
            ...
            switch (type) {
                case GET:
                    keyOperateCallBack.actionChange(SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_take_over)
                            + "：【GET】 【" + keyInfo.keyName + "】 " +
                            SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_success)
                            + "：" + result);
                    break;
                case SET:
                    keyOperateCallBack.actionChange(SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_take_over)
    ...
    private void endFail(int type, @NonNull IAutelCode error, @Nullable String errorMsg) {
        if (keyOperateCallBack != null) {
            String errString = SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_failed)
                    + "：" + SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_error_code_is)
                    + "：" + error + ","
                    + SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_error_message)
                    + "：" + errorMsg;
    ...
    public void doSet(String jsonStr) {
        try {
            ...
            ToastUtils.INSTANCE.showToast(SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_parameter_input_error));
        }
    }
    ...
    private P validPrams(String jsonStr) {
        if (Util.isBlank(jsonStr)) {
            String msg = "【SET】【" + getName() + "】【" + keyInfo.keyName + "】 "
                    + SDKManager.get().getSContext().getString(com.autel.drone.demo.R.string.debug_please_set_parameters_first);
            if (keyOperateCallBack != null) {
